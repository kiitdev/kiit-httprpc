...
> **Why this import step matters:** `kiit-rpc-kotlin/kiit-rpc/build.gradle.kts`'s
...
> keyring (`~/.gnupg/`) and selects/unlocks the key via the `signing.gnupg.keyName` /
> `signing.gnupg.passphrase` properties (see [Setup](#setup) below). So the import above is what
> actually makes the key available — Gradle itself never touches the raw key material. This is
> also why `useGpgCmd()` is required instead of `useInMemoryPgpKeys` — see the
> ["Could not read PGP secret key"](#could-not-read-pgp-secret-key) FAQ entry.
...
```bash
./gradlew :kiit-rpc:publishAndReleaseToMavenCentral
```
...
|----------|------|---------|---------------|
| CI | `ci.yml` | Every PR into `main` | `ktlintCheck`, `detekt`, `jvmTest` on `ubuntu-latest`; iOS simulator tests on `macos-latest`. |
...
1. Bump `libraryVersion` in [`kiit-rpc-kotlin/kiit-rpc/build.gradle.kts`](./kiit-rpc-kotlin/kiit-rpc/build.gradle.kts)
...
3. It reads the version via `./gradlew :kiit-rpc:printVersion`, verifies a tag for that
...
device` during import; on Linux it more often just hangs. The release workflow's "Import GPG key"
step already forces loopback pinentry mode before importing:
...
# Clean build outputs
./gradlew :kiit-rpc:clean

# Compile all targets (JVM, Android, iOS)
./gradlew :kiit-rpc:build

# Compile only — no tests
./gradlew :kiit-rpc:assemble
...
./gradlew :kiit-rpc:jvmTest

# All platform tests
./gradlew :kiit-rpc:allTests

# Publish to Maven Local (~/.m2) for integration testing against other modules
./gradlew :kiit-rpc:publishToMavenLocal
```

Maven Local artifacts are saved to:
```
~/.m2/repository/dev/kiit/kiit-rpc/
```
...
[central.sonatype.com/artifact/dev.kiit/kiit-rpc](https://central.sonatype.com/artifact/dev.kiit/kiit-rpc)
...
```bash
./gradlew :kiit-rpc:signKotlinMultiplatformPublication
```
...
`kiit-rpc-kotlin/kiit-rpc/build.gradle.kts`:
```kotlin
val libraryVersion = "0.0.0"   // ← bump here
```
